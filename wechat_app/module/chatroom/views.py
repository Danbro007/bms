...
from wechat_app.module.wechat_push.models import WechatPush
    ...
    if req.method == "GET":
        ...
        if end_time - start_time <= 0:
            ...
            try:
                ...
                # push_list = WechatPush.objects.filter(chatroom_id=id).only("id")
                # if push_list:
                #     start_time = req.POST.get("start_time")
                #     end_time = req.POST.get("end_time")
                #     for id in push_list:
                #         sched.modify_job(id=str(id), start_date=start_time, end_date=end_time)
                print("群推送时间修改成功")
            except IntegrityError:
                response["code"] = 400
