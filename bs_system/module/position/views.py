...
def PositionData(req):
    ...
    if keyword:
        positions_all = Position.objects.filter(name__contains=keyword, mark=1).all()
    else:
        positions_all = Position.objects.filter(mark=1).all()
    count = positions_all.count()
    positions = positions_all[(page - 1) * limit:page * limit]
    positions_data = []
