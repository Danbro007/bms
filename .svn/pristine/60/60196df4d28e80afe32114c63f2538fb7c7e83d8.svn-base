...
def RoleData(req):
    ...
    if keyword:
        roles_all = Role.objects.filter(name__contains=keyword, mark=1).all()
    else:
        roles_all = Role.objects.filter(mark=1).all()
    count = roles_all.count()
    roles = roles_all[(page - 1) * limit:page * limit]
    roles_data = []
    for role in roles:
        role_info = {
            "id": role.id,
